...
const DisplayData = () => {
  ...
  useEffect(() => {
    if (countryNameInput) {
      // Fetchni to jen dyž tam něco je
      const fetchData = async () => {
        try {
          const response = await fetch(
            `https://restcountries.com/v3.1/name/${countryNameInput}`
          );
          ...
          const matchingCountry = data.find((country: any) =>
            country.name.common
              .toLowerCase()
              .startsWith(countryNameInput.toLowerCase())
          );

          if (matchingCountry) {
            setCountryName(matchingCountry.name.common);
            setCountryCapital(
              matchingCountry.capital ? matchingCountry.capital[0] : "N/A"
            );
            setCountryPopulation(matchingCountry.population);
            setCountryCode(matchingCountry.cca2);
            setFlag(
              `https://flagcdn.com/${matchingCountry.cca2.toLowerCase()}.svg`
            );
          }
        } catch (error) {
  ...
  return (
    <div className={DisplayDataCSS.main__container}>
        <h1 className={DisplayDataCSS.title}>CountryPedia</h1>
      <div className={DisplayDataCSS.container}>
        <div className={DisplayDataCSS.country__info__container}>
          <input
            ...
            type="text"
          />
          <p className={DisplayDataCSS.country__info}>
            <b>Capital city: </b> {countryCapital}
          </p>
          <p className={DisplayDataCSS.country__info}>
        ...
        <div className={DisplayDataCSS.flag__container}>
          {flag && (
            <img src={flag} alt="flag" className={DisplayDataCSS.flag} />
          )}
        </div>
      </div>
    </div>
